...
- ODX uses the integrated DEX on Chain $C$ (e.g., Uniswap) to swap Asset $A$ to USDT/USDC.
...
- ODX treats USDT on Chain $C$ and Chain $D$ as interchangeable.
...
- ODX uses the integrated DEX on Chain $D$ (e.g., Raydium) to swap USDT to Asset $B$.
...
- The user's ODX account is credited with $B_D$.
